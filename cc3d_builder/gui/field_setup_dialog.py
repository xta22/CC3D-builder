...
class FieldSetupDialog(QDialog):
    ...
    def setup_bc_tab(self):
        ...
        for axis in ["X", "Y", "Z"]:
            ...
            # Disable min/max inputs when Periodic is selected.
    ...
    def accept_data(self):
        """Collect UI data → normalize → store in self.field_data"""
        ...
        try:
            ...
            for row in range(self.table_chemo.rowCount()):
                ...
                # only saturation / saturation linear modes need SatCoef
    ...
    def load_data_into_ui(self):
        ...
        # 2. All subsequent get operations should target data (a confirmed dictionary), no longer self.initial_data
        ...
        bc_data = data.get("BoundaryConditions", data.get("boundary_conditions"))
        ...
        # --- 3. Chemotaxis ---
        raw_chemo = data.get("Chemotaxis", data.get("chemotaxis"))
        ...
        for entry in chemo_list:
            ...
            if isinstance(cb_cell, QComboBox):
                cb_cell.setCurrentText(entry.get("CellType", entry.get("cell_type", "")))
            
            lambda_item = self.table_chemo.item(row, 1)
            if lambda_item:
                lambda_item.setText(str(entry.get("Lambda", entry.get("lambda", "100.0"))))
            
            cb_type = self.table_chemo.cellWidget(row, 2)
            if isinstance(cb_type, QComboBox):
                cb_type.setCurrentText(entry.get("Type", entry.get("mode", "regular")))
            
            sat_item = self.table_chemo.item(row, 3)
            if sat_item:
                sat_item.setText(str(entry.get("SatCoef", entry.get("sat_coef", "0.0"))))

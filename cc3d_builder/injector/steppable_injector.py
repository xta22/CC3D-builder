...
class SteppableInjector:
    ...
    def _remove_marker_block(self, content, marker):
        full_marker_start = f"# === {marker} START ==="
        full_marker_end = f"# === {marker} END ==="
        ...
        for line in lines:
            if full_marker_start in line:
                ...
                changed = True
                continue
            if full_marker_end in line and skip:
                skip = False
                continue
            if not skip:
                new_lines.append(line)
    ...
    def remove_volume_start_code(self, celltype_name):
        content = self._read_file()
        marker = f"CC3D_VOLUME_{celltype_name.upper()}"
        new_content, changed = self._remove_marker_block(content, marker)
        if changed:
            self._write_file(new_content)
            print(f"[Injector] Volume init removed for {celltype_name}")
        return changed
